...
gulp.task('sass', function(){ // Создаем таск Sass
    return gulp.src('app/sass/main.sass') // Берем источник
        .pipe(sass({ // Преобразуем Sass в CSS посредством gulp-sass
            includePaths: require('node-bourbon').includePaths // Подключаем Bourbon в main.sass
...
gulp.task('watch', ['browser-sync', 'css-libs', 'scripts'], function() {// Создаем таск "watch". Таски sass и browser-sync выполняем до его запуска)
    gulp.watch('app/sass/**/*.sass', ['sass']); // Gulp наблюдает за всеми sass файлами и при их сохранении выполняет таск sass
...
gulp.task('build', ['clean', 'img', 'sass', 'scripts'], function() {
    ...
    var buildMail = gulp.src('app/mail.php') // Переносим mail.php в продакшен
    .pipe(gulp.dest('dist'));
